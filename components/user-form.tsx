...
export default function UserForm(props: UpdateUserFormProps) {
  ...
  const validateForm = (): boolean => {
    ...
    if (form.name.trim().length < 3) newErrors.name = 'O nome deve ter pelo menos 3 caracteres.';
    ...
    if (!props.hasUser) {
      ...
      if (form.password !== confirmPassword) newErrors.confirmPassword = 'As senhas não coincidem.';
  ...
  const validatePasswordForm = (): boolean => {
    const newErrors: { [key: string]: string } = {};
    if (!passwordForm.newPassword.trim()) newErrors.newPassword = 'Nova senha é obrigatória';
              ...
              {!props.hasUser && (
                <>
                  ...
                  <View style={styles.securitySection}>
                    ...
                    <TouchableOpacity
                      style={[styles.dropdownButton, errors.securityQuestion && styles.inputError]}
                      onPress={() => setShowQuestionsDropdown(!showQuestionsDropdown)}
                    >
                      <Text
                        style={selectedQuestion ? styles.dropdownText : styles.dropdownPlaceholder}
                      >
