...
]; //Trocar o sistema de cache padrão pelo FileStore customizado, apontando para o cache fora do OneDrive
...
config.server = {
    ...config.server,
    enhanceMiddleware: (middleware, server) => { //Cria um novo enhanceMiddleware (um gancho que intercepta requisições HTTP internas feitas ao servidor Metro)
        const wrapped = originalEnhanceMiddleware ? originalEnhanceMiddleware(middleware, server) : middleware; //Mantém o middleware original funcionando (se existe)

        return (req, res, next) => { //Retornaa uma função que será chamada a cada requisição interna feita ao servidor Metro
            if (req.url?.startsWith("/symbolicate")) { //Bloqueia a symbolication
